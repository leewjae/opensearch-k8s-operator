...
The operator is developed in Go, as such you need a current Go toolkit (for Linux most distributions provide packages, otherwise get it from the [go homepage](https://go.dev/)). Please use version 1.25.8 as this version is used by the CI pipelines. You also need an editor/IDE, ideally with Go support. We recommend either [VS Code](https://code.visualstudio.com/) with the official Go extension or [GoLand](https://www.jetbrains.com/go/).
...
* If you make changes to the CRD the CRD YAMLs must be updated (via `make manifests`) and also copied into the helm chart:
    ...
    ```bash
    cp opensearch-operator/config/crd/bases/opensearch.dremio.io_*.yaml charts/opensearch-operator/files/
    ```
