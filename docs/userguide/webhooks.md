...
- **Suffix `.opensearch.dremio.io`**: Uses the same domain as the operator's API group (`opensearch.dremio.io`), ensuring consistency across the operator's resources

For example:
- `vopensearchcluster.opensearch.dremio.io` - Validating webhook for OpenSearchCluster resources
- `vopensearchuser.opensearch.dremio.io` - Validating webhook for OpenSearchUser resources
...
Webhooks are disabled by default to keep existing deployments working without extra dependencies (such as cert-manager). You can enable them via the `webhook.enabled` value, which also drives the operator's `--enable-webhooks` flag:
...
Cert-manager automatically generates and manages TLS certificates for the webhooks. When you enable webhooks you can also enable this integration via `webhook.certManager.enabled=true`.
